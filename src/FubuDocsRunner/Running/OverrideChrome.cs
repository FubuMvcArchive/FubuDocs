...
namespace FubuDocsRunner.Running
{
    ...
    public class OverrideChrome : IConfigurationAction
    {
        ...
        {
            var file = graph.Files.FindFiles(FileSet.Shallow("TopicChrome.spark"))
            ...
            if (file != null)
            {
                ...
                var descriptor = new ViewDescriptor<Template>(template) {ViewModel = typeof (TopicChrome)};

                var sparkDescriptor = new SparkDescriptor(descriptor.Template) {ViewModel = typeof (TopicChrome)};

                var view = new SparkViewToken(sparkDescriptor);

                var chain = graph.Behaviors.Single(x => x.InputType() == typeof(TopicChrome));
                chain.Output.ClearAll();
                chain.Output.Writers.AddToEnd(new SpecialView<ChromeContent>(view));
            }
